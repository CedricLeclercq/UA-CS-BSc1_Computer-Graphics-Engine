...
img::EasyImage GenerateImage::generate_image(const ini::Configuration &configuration)   {
    ...
    if (typeString == "2DLSystem") {
        ...
        if (typeString == "ZBuffering" or typeString == "LightedZBuffering") {
            ...
            double shadowMask = configuration["General"]["shadowMask"].as_double_or_default(0);
            // TODO check if default value for shadow mask is ok
    ...
    // If no image was created or returned, return this empty default image
    img::EasyImage emptyImage(1024,1024);
    return emptyImage;
}
